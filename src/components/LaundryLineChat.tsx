...
export type LaundryLineChatController = {
  isOpen: boolean;
  open: () => void;
  close: () => void;
...
export function useLaundryLineChat(
  metadataSource: string = "landing-cta",
): LaundryLineChatController {
...
type LaundryLineChatModalProps = {
  controller: LaundryLineChatController;
  title?: string;
  description?: string;
};

export function LaundryLineChatModal({
  controller,
  title = "Snow White Laundry — Laundry Line",
  description = "Ask us about visiting, private events, or how the room will work.",
}: LaundryLineChatModalProps) {
  const {
    isOpen,
    close,
    messages,
          ...
          >
            <header className="mb-6 flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-4">
              <div>
                ...
                <h3 className="mt-1 text-2xl font-light text-white">Laundry Line</h3>
...
export function LaundryLineChat() {
  const controller = useLaundryLineChat("landing-cta");
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.02] px-6 py-8 shadow-[0_20px_80px_rgba(0,0,0,0.45)] backdrop-blur">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-light text-white">Talk to the room</h2>
