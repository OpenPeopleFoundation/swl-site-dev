...
import { FoodItemCard } from "@/domains/staff/inventory/food/FoodItemCard";
import { FoodCountPane } from "@/domains/staff/inventory/food/FoodCountPane";
import { FoodReceivingPane } from "@/domains/staff/inventory/food/FoodReceivingPane";
import { FoodVendorPanel } from "@/domains/staff/inventory/food/FoodVendorPanel";
import { FoodAutoReplenishPanel } from "@/domains/staff/inventory/food/FoodAutoReplenishPanel";
import { FoodForecastPanel } from "@/domains/staff/inventory/food/FoodForecastPanel";
import { FoodStorageMapViewer } from "@/domains/staff/inventory/food/FoodStorageMapViewer";
import { FoodWasteModal } from "@/domains/staff/inventory/food/FoodWasteModal";
...
export function FoodInventoryDashboard({
  ...
  const handleReceive = useCallback(
    async (payload: {
      ...
      setIsReceiving(true);
      try {
        const response = await fetch("/api/inventory/food/receive", {
          method: "POST",
  ...
  const handleCount = useCallback(
    async (payload: { itemId: string; quantity: number }) => {
      setIsCounting(true);
      try {
        const response = await fetch("/api/inventory/food/count", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            itemId: payload.itemId,
  ...
  const handleWaste = useCallback(
    async (payload: { itemId: string; quantity: number; reason?: string; notes?: string }) => {
      try {
        const response = await fetch("/api/inventory/food/waste", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            itemId: payload.itemId,
