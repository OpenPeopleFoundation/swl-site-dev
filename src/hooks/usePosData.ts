...
export function usePosData() {
  ...
  const openCheck = useCallback(
    async (tableSlugs: string[]) => {
      ...
      const response = await fetch(`/api/pos/tables/${encodeURIComponent(primary)}/checks`, {
        method: "POST",
  ...
  const addLine = useCallback(
    async (checkId: string, payload: AddLinePayload) => {
      const response = await fetch(`/api/pos/checks/${encodeURIComponent(checkId)}/lines`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const updateCheck = useCallback(
    async (checkId: string, payload: UpdateCheckPayload) => {
      const response = await fetch(`/api/pos/checks/${encodeURIComponent(checkId)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
